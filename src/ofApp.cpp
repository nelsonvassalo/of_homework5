...
// constructor
agent::agent(){	
  velDir.set(ofRandom(0.1, 3), ofRandom(0.1, 2));	// init random speed
...
void agent::update(agent *arr){  // influence the main vector 
  ...
  if (middle != noVec) {
    ...
    velDir.rotate(alignMouse*.05);			// apply centroid gravity (weighted)
...
void agent::draw(){
  ...
  // if it moves off the board, move them back to center
    if (location.x > ofGetWidth()) location.set(ofGetWidth()/2, ofGetHeight()/2);
  if (location.x < 0) location.set(ofGetWidth()/2, ofGetHeight()/2);
  if (location.y > ofGetHeight()) location.set(ofGetWidth()/2, ofGetHeight()/2);
  if (location.y < 0) location.set(ofGetWidth()/2, ofGetHeight()/2);
...
float agent::calcAlignment(agent *arr, int max){
  ...
  for (int i=0; i < max; i++) {
    ...
    if (distance < 10 && distance > 0 && arr[i].active) { 	// if a neighbor is close & active
      //cout  << " Dist: " << distance << "\n";
...
ofVec2f agent::calcCohesion(agent *arr, int max){
  ...
  for (int i=0; i < max; i++) {
    ...
    if (distance < 40 && distance > 20 && arr[i].active) { 	// if a neighbor is close & active
...
ofVec2f agent::calcDispersion(agent *arr, int max){
  ...
  for (int i=0; i < max; i++) {
    ...
    if (distance < 20 && arr[i].active) { 	// if a neighbor is close & active 
      ...
      float distanceToMouse = mouse.distance(arr[i].location);
      
      if(distanceToMouse < 100 && distanceToMouse > 0 && arr[i].active) {
          float mouseAngle = velDir.angle(mouse - location);
          velDir.rotate(mouseAngle * 0.05);
          activeColor = ofColor(0,255,0);
          ofSetColor(255, 0, 0);
        } else {
          activeColor = ofColor (255,255,255,150);
      }
